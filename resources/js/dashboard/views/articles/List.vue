<template>
  <a-card>
    <div class="tableList">
      ...
      >
        <template #article_title="title, article">
          <router-link :to="{ name: 'article.edit', params: {id: article.id}}">
            {{ title }}
          </router-link>
        </template>

        <template #article_user="user, article">
          <router-link :to="{ name: 'article.index', query: {user: user.id}}">
            {{ user.name }}
          </router-link>
        </template>

        <template #article_category="category">
          <router-link
            :to="{ name: 'article.index', query: {category: category.id}}"
          >
            {{ category.name }}
          </router-link>
        </template>
        ...
        <template #article_tags="tags">
          <template v-for="tag in tags">
            <a-tag :key="tag.id" color="blue">
              <router-link :to="{ name: 'article.index', query: {tag: tag.id}}">
                {{ tag.name }}
              </router-link>
            </a-tag>
          </template>
...
const columns = [
  ...
  },
  {
    ...
    dataIndex: 'user',
    width: 100,
    scopedSlots: { customRender: 'article_user' }
  },
  {
    title: '分类',
    dataIndex: 'category',
    width: 120,
    scopedSlots: { customRender: 'article_category' }
  },
  {
    title: '标签',
    dataIndex: 'tags',
    width: 300,
    scopedSlots: { customRender: 'article_tags' }
  },
  {
    title: '状态',
    dataIndex: 'status',
    width: 120,
    scopedSlots: { customRender: 'article_status' }
  },
  {
    title: '总数',
    width: 120,
