...
$(document).ready(function ($) {
  ...
  $(window).on('load',
    function () {
      $('.main-preloader').length && ($('.main-preloader').addClass('window-is-loaded'), setTimeout(function () {
...
var $window, $html, $pageHeader, $pageHeader_height, $window_height, $window_width, $searchHeader, $search_down, $search_down_title, $html_overlay
...
function scrollTop () {
  ...
  var startPoint = $window_height / 2
  var scrTopBtn = $('.scroll-to-top')
...
function showing_search () {
  ...
  $('body').removeClass('nt_second_menu_search_anims'),
  $('body').removeClass('nt_second_menu_height_100'),
  $('.featured-search').removeClass('hidden'),
  setTimeout(function () {
    $('body').removeClass('nt_showing_search')
  },
  600)
}
...
function search_height () {
  ...
  if (w_height < 768) {
    $('.featured-search').addClass('hidden'),
    $('body').addClass('nt_second_menu_height_100')
  }
...
$('.btn-search').off('click').on('click',
  function () {
    ...
    $('body').addClass('nt_showing_search'),
    $('body').addClass('nt_second_menu_search_anims'),
    $('.top_searchform_wrapper input').focus(),
    search_height()
  }),

document.addEventListener('keyup',
  function (t) {
    t.keyCode === 27 && ($('body').hasClass('nt_second_menu_search_anims') && showing_search())
  }),
$('.search_inner_close').on('click',
  function () {
    showing_search()
  })
...
function toggleCommentAuthorInfo () {
  ...
  $('.comment-form-info').slideToggle('slow',
    function () {
...
$(document).ready(function ($) {
  ...
  if (globals.silide != 0) {
    ...
    featuredowl.on('mousewheel', '.owl-stage',
      function (e) {
        if (e.deltaY < 0) {
  ...
  */
  ...
  $('#mobile-menu-icon').on('click touchstart',
    function (e) {
      e.preventDefault()
      $('#nice-mobile-overlay').addClass('open')
      $('body').addClass('mobile-open')
    })
  $('#mobile-close-icon').on('click touchstart',
    function (e) {
      e.preventDefault()
  ...
  */
  ...
  if ($window_width < 768) {
    $('.btn-open-share').on('click touchstart',
      function (e) {
        e.preventDefault()
    ...
    $('.btn-close-share').on('click touchstart',
      function (e) {
        e.preventDefault()
  ...
  $.fn.miPopup = function () {
    this.bind('click touchstart',
      ...
      // event.preventDefault();
        ...
        $(selector).addClass('open').find('.btn-close').on('click touchstart',
          function (event) {
            event.preventDefault()
            $(html).remove()
...
if ($('.posts-ajax-load').length > 0) {
  addViewportEvent(document.querySelector('.posts-ajax-load'),
    function () {
...
$(document).on('click', '.dposts-ajax-load',
  function (event) {
    event.preventDefault()
    if ($('.dposts-ajax-load').hasClass('loading') === false) {
      $('.dposts-ajax-load').addClass('loading')
  ...
  $.ajax({
    ...
    if (response) {
      if (data.commentspage == 'newest') {
        $('.dposts-ajax-load').data('paged', data.paged * 1 - 1)
      } else {
        $('.dposts-ajax-load').data('paged', data.paged * 1 + 1)
      }
      ...
      $('.dposts-ajax-load').removeClass('loading').show()
    } else {
      $('.dposts-ajax-load').addClass('disabled').text('没有更多内容').show()
    }
  }).fail(function () {
    console.log('error')
    $('.post-loading').hide()
...
$(document).on('click', '.comment-action',
  function (event) {
    event.preventDefault()
    ...
    if (t.parent().find('a').hasClass('current')) {
      if (t.hasClass('current')) {
        if (data.caction == 'up') {
          mi_prompt('error', '你已经给过好评了')
        } else {
          mi_prompt('error', '你已经给过差评了')
        }
        return false
      }
      mi_prompt('error', '不能重复操作！')
      return false
    }
...
$(document).on('click', '.captcha-image img',
  function (event) {
    event.preventDefault()
...
function popupGallery (gclass, aclass) {
  var gclass = gclass || '.suxing-popup-gallery'
  var aclass = aclass || 'a.suxing-popup-gallery-item'
...
function mi_comment_hide () {
  $.each($('.children'), function (index, val) {
    let li_count = $(this).children('li').length
    const show_count = li_count - 2

    if (li_count > 2 && $(this).children('.load-more-comment').length < 1) {
      $(this).children('li').each(function (index, el) {
        if (index === 1) {
          $(this).after('<span data-count="' + show_count + '" class="load-more-comment o"><button class="btn">还有' + show_count + '条回复 <i class="fal fa-angle-down" aria-hidden="true"></i></button></span>')
        }
      })
    }
  })
...
$(document).on('click', '.load-more-comment', function (event) {
  ...
  if (t.hasClass('o')) {
    ...
    t.children('.btn').html('收起 <i class="fal fa-angle-up" aria-hidden="true"></i>')
  } else {
    t.removeClass('c').addClass('o')
    t.children('.btn').html('还有' + t.data('count') + '条回复 <i class="fal fa-angle-down" aria-hidden="true"></i>')
  }
})
...
$(document).on('click', '.filter-menu button', function (event) {
  ...
  if (!t.hasClass('active')) {
    ...
    $('.home-list').html('')
    $('.dposts-ajax-load').addClass('loading').text('加载更多')
    ajax_load_posts($('.dposts-ajax-load').data())
  }
})

$(document).on('click touchstart', '.btn-bigger-cover',
  function (event) {
    event.preventDefault()
...
$(document).on('click', '.page-break-ajax',
  function (event) {
    event.preventDefault()
...
$(document).on('click', '.single-weixin',
  function (event) {
    event.preventDefault()
    var img = $(this).data('img')
    $('.bdsharebuttonbox').append('<div class="dialog-suxing" id="post_qrcode">\
...
function menu_item_hidden () {
  ...
  var right = $('.m-header').width() + $('.m-header').offset().left

  if (right < $('.navbar-site > li:nth-last-child(-n+1)').offset().left + 150) {
    var i = 1
    while (true) {
      var hiddenMenus = $('.navbar-site > li:nth-last-child(-n+' + i + ')')
    ...
    var collapseMenus = '<li class="menu-item menu-item-has-children has-children hidden-nav"><a href="#"><i class="far fa-ellipsis-h"></i></a>' + '<ul class="sub-menu">' + $('<div/>').append(hiddenMenus.clone()).html() + '</ul>' + '</li>'
...
$(window).on('resizeend',
  function (e) {
    var navbarSite = $('.navbar-site')
    navbarSite.append($('.hidden-nav .sub-menu li').clone())
    $('.hidden-nav').remove()
