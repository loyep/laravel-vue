...
$footer_style = 1;
//$footer_info = \App\Support\Helper::getWidget('footer_info');
$site_info = get_option('site_info', []);
$footer_year = get_footer_year($site_info->year);
?>

@inject('site', 'App\Services\Site\Site')
{{--@if ($footer_style === 2)--}}
...
{{--@if(!empty($footer->miitbeian))--}}
{{--<a href="http://www.miitbeian.gov.cn/" target="_blank">{{ $footer->miitbeian }}</a>--}}
{{--@endif--}}
                ...
                <div class="col-12 col-lg-5 mb-5 mb-lg-0">
                    @if ($site->footerInfo())
                        <aside class="footer-widget footer-widget-text">
                            <h3 class="widget-title">{{ $site->footerInfo()->title }}</h3>
                            <div class="text-widget">
                                <p>{{ $site->footerInfo()->text }}</p>
                            </div>
                        </aside>
                    @endif
        ...
        <div class="footer-bottom">
            <div class="row">
                <div class="col-12 col-md-12 footer-copyright">
                    <p class="text-secondary">Copyright © {{ $footer_year }}
                        <a href="{{ url('/') }}">{{ $site_info->name }}</a>.
                        @if(!empty($footer->miitbeian))
                            <a href="http://www.miitbeian.gov.cn/" target="_blank">{{ $footer->miitbeian }}</a>
                        @endif
                        Designed by <a href="https://github.com/loyep/grace" target="_blank">Grace</a>.</p>
                </div>
            </div>
        </div>
    </div>
